...
const Products = () => {
  ...
  const { order, orderByPrice, handleOrder, handleOrderByPrice, sortedList } = useSorting(components);
  const { pageNumber, pageRangeDisplayed, handlePageClick, paginatedList, itemsPerPage } = usePagination(sortedList());

  return (
    <section id='products' className='px-[30px] py-[40px] md:px-[20px] 2xl:px-0 font-main max-w-[1050px] m-auto'>
      <div id="products">
        <h1 className='text-[20px] md:pl-[20px] 2xl:pl-0'>PRODUCTOS</h1>
      </div>
